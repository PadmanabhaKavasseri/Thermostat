...
void lcd_init(void)
{
    ...
    lcd_writenibble((unsigned char) 0b0011 << 4);                   // Use lcd_writenibble to send 0b0011
    _delay_ms(5);               // Delay at least 4msec

    lcd_writenibble((unsigned char) 0b0011 << 4);                   // Use lcd_writenibble to send 0b0011
    _delay_us(120);             // Delay at least 100usec

    lcd_writenibble((unsigned char) 0b0011 << 4);                   // Use lcd_writenibble to send 0b0011, no delay needed

    lcd_writenibble((unsigned char) 0b0010 << 4);                  // Use lcd_writenibble to send 0b0010
...
{
   PORTD &= 0x0f;
   PORTD |= (lcdbits & 0xf0);  /* Load PORTD, bits 7-4 with bits 7-4 of "lcdbits" */
